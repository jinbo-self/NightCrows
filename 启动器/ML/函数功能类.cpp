...
void 持续技能开启(识字类 识字, 键鼠类 键鼠, 识图类 识图)
{
    ...
    while (!识字.查找技能_失明())
    {
        ...
        for (int i = 0; i < 236; i++)
        {
            ...
            识字坐标 当前坐标 = { 1176 ,119 + i*2,1198,603 + i*2 };
            std::wstring 字符 =  识字.获取字符(当前坐标);//重新设计查找决心，精准射击，开始狩猎
            if (字符.find(L"关闭")!= std::wstring::npos
                || 字符.find(L"关") != std::wstring::npos
                || 字符.find(L"闭") != std::wstring::npos)
            {
                键鼠.移动鼠标((当前坐标.x1+ 当前坐标.x2)/2, (当前坐标.y1, 当前坐标.y2)/2); //主动技能栏
                键鼠.点击鼠标左键();
                Sleep(10);
            }
        }
